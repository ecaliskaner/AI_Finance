...
3. **Dead man's switch** — if the process stops heartbeating for 15 minutes,
   alert loudly. Consider exchange-side stop-losses as a backstop so a crashed
   process doesn't leave a position unmanaged.
...
| Risk | Mitigation |
|---|---|
| VPS dies mid-position | Exchange-side stop-loss as backstop; state reconciliation on restart |
...
| Code bug in a new deploy | Deploy to paper first for 48 hours, always |
...
- **Overconfidence after a winning streak.** Three good weeks is noise. The
  capital ladder (PLAN §9.4) must be written down *before* going live,
...
- [ ] Crash recovery tested — process killed mid-position, restarts correctly
